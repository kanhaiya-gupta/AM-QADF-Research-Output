...
Transforming points before voxelization avoids resampling artifacts and preserves point-level precision. Downstream signal mapping (Paper 1) and fusion operate on already-aligned points, so alignment quality is not limited by voxel resolution. Validation is performed on the same bbox corners used for fit selection, so pass/fail is consistent and interpretable.
