...
## Relation to Signal Mapping (Paper 1)

Signal mapping (Paper 1) transforms aligned point-based data into a voxel grid via interpolation (nearest, linear, IDW, KDE, RBF). It assumes spatially and temporally aligned data (Paper 0) and *optionally* processed and corrected signals (this paper). Paper 1 references calibration and correction as prerequisites; this paper (Paper 0.5) provides their design, algorithms, and implementation. There is no duplication: Paper 0.5 focuses on *what* values to use; Paper 1 focuses on *where* to put them.
