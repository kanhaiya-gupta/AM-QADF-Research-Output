...
Processing and correction sit **after** spatial and temporal alignment (Paper 0) and **before** (or alongside) signal mapping (Paper 1):

1. **Paper 0**: Align points and time/layer → unified coordinate system and consistent time reference.
2. **Paper 0.5**: Process and correct signals → noise reduction, calibration, distortion correction.
3. **Paper 1**: Map processed/aligned signals to voxel domain → interpolation, grid generation.
...
flowchart TB
    ...
    subgraph P1["Paper 1: Signal Mapping"]
...
- **Post-mapping / pre-fusion / post-fusion**: Optional; apply when residual errors are detected at a later stage. Paper 1 describes these options; this paper describes the correction models and validation.
